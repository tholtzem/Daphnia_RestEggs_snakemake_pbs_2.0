...
#samples_info = pd.read_csv("list/21.list", sep='\t', index_col=False)
...
#sample_prefix = list(samples['prefix'])
#sample_id = list(samples['sample_id'])

# load single samples for saf
saf_samples = pd.read_csv("list/Da_Resteggs_metadata_cleaned_sorted_LC_REF.csv", sep='\t', index_col=False)
saf_prefix = list(saf_samples['prefix'])
