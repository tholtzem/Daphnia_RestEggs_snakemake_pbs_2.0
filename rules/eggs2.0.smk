...
rule fastqc_trm:
  input:
    ...
    r2 = "trm/{sample}_R2.trmdfilt.fq.gz"
  output:
    #touch("trm/qc/fastqc/fastqc_{sample}.done")
...
rule samtools_Refclones_minq20:
  input:
  ...
  message: """ --- Exclude unmapped reads and only keep reads with minq => 20 from reference clones --- """
  shell:
    """
    /home/uibk/c7701178/.conda/envs/eggs2.0/bin/samtools view -F 4 -Shu -q 20 {input} | samtools sort - -o {output} 2> {log}
    """

rule clip_overlap:
  input:
    deDup = 'deDup/{sample}.dedup.bam',
    log_files = 'log/{sample}.ref_minq20.log'
  output:
...
rule ref_Dict:
  input:
    ref = config['ref_rapid']
  output:
    'ref/dgal_ra_pb-target-and-other_ill-confilter.blobfilter.rmmt_sspace-lr3_lrgc3_pg3_pilon_3.simple.fasta.dict'
