...
## plot PCA with IDs
pca = ggplot(data = df, aes(x=V2, y=V3, label=pop)) + geom_point(pch=16, size=4.5) + ggtitle("PC1 vs PC2 (LDpruned SNPs)") + geom_text()
pca = pca + xlab(PC1) + ylab(PC2) + theme + scale_color_manual(values = cbp2)
pca

## PC1vsPC2
pca = ggplot(data = df2, aes(x=V2, y=V3, color=groups)) + geom_point(pch=16, size=4.5) + ggtitle("PC1 vs PC2 (LDpruned SNPs)")
pca = pca + xlab(PC1) + ylab(PC2) + theme + scale_color_manual(values = cbp2)
pca

## PC1vsPC3
pca = ggplot(data = df2, aes(x=V2, y=V4, color=groups)) + geom_point(pch=16, size=4.5) + ggtitle("PC1 vs PC3 (LDpruned SNPs)")
pca = pca + xlab(PC1) + ylab(PC3) + theme + scale_color_manual(values = cbp2)
pca

## PC2vsPC3

pca = ggplot(data = df2, aes(x=V3, y=V4, color=groups)) + geom_point(pch=16, size=4.5) + ggtitle("PC2 vs PC3 (LDpruned SNPs)")
